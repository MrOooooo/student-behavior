...
class EyeTracker:
    ...
    def process_frame(self, frame, alert_counter, frame_counter,
                      ...
                      is_video=False, selected_target="All", selected_targets=None):
        alerts = []
        ...
        try:
            ...
            if detection_result.face_landmarks:
                ...
                if (left_conf > cnn_threshold and left_direction != "closed") or \
                    ...
                    target_matched = (
                                             self._target_selected(left_direction,
                                                                   active_targets) and left_conf > cnn_threshold
                                     ) or (
                                             self._target_selected(right_direction,
                                                                   active_targets) and right_conf > cnn_threshold
                                     )
                ...
                if self._target_selected(left_direction, active_targets):
                ...
                if self._target_selected(right_direction, active_targets):
    ...
    @staticmethod
    def _normalize_targets(selected_target="All", selected_targets=None):
    ...
    @staticmethod
    def _target_selected(direction: str, active_targets) -> bool:
        return not active_targets or direction in active_targets

    def process_eye_detections(self, frame, alert_counter, frame_counter,
                               cnn_threshold=0.6,
                               duration_threshold=5.0, is_video=False, selected_target="All", selected_targets=None):
        """
        ...
        try:
            ...
            if active_targets:
                if left_direction not in active_targets:
                    left_conf = 0.0  # Ignore left eye if it doesn't match the target
                if right_direction not in active_targets:
            ...
            if not detection_result.face_landmarks:
                ...
                return processed_frame, alerts, total_detections, process_time, "N/A", 0, coords
