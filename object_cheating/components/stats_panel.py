...
def stats_panel() -> rx.Component:
    ...
    def cross_target_checkbox(model_num: int, targets_state, target: str) -> rx.Component:
        return rx.hstack(
            rx.checkbox(
                ...
                on_change=lambda checked: CameraState.toggle_cross_model_target(
                    model_num,
                    target,
                    checked,
                ),
    ...
    def cross_model_group(
        ...
        return rx.box(
            ...
            rx.flex(
                *[
                    cross_target_checkbox(model_num, targets_state, target)
    ...
    single_model_selector = rx.vstack(
        ...
        rx.flex(
            rx.foreach(target_classes, target_checkbox),
            wrap="wrap",
            gap="2",
            max_width="320px",
        ),
        rx.text(
            f"Selected: {CameraState.selected_target}",
            class_name="text-xs text-cyan-300/75",
        ),
        spacing="2",
        align="start",
    )
    ...
    cross_model_selector = rx.vstack(
        rx.hstack(
            rx.text("跨模型检测", class_name="text-cyan-100 font-medium"),
            rx.switch(
        ...
        rx.cond(
            ...
            rx.vstack(
                cross_model_group(
                    ...
                    1,
                    model1_classes,
                    CameraState.model1_cross_targets,
                ),
                cross_model_group(
                    "Model 2",
                    2,
                    model2_classes,
                    CameraState.model2_cross_targets,
                ),
                cross_model_group(
                    "Model 3",
                    3,
                    model3_classes,
                    CameraState.model3_cross_targets,
                ),
                cross_model_group(
                    "Model 4",
                    4,
                    model4_classes,
                    CameraState.model4_cross_targets,
                ),
                cross_model_group(
                    "Model 5",
                    5,
                    model5_classes,
                    CameraState.model5_cross_targets,
                ),
                spacing="3",
                width="100%",
            ),
    ...
    return rx.vstack(
        rx.el.h3("检测模型设置", class_name="text-lg font-semibold mb-2 text-cyan-50"),
        rx.hstack(
            rx.text(
