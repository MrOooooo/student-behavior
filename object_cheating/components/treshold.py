...
def threshold() -> rx.Component:
    ...
    def cross_model_thresholds() -> rx.Component:
        ...
        model6_card = rx.box(
            ...
            threshold_row("Confidence", CameraState.model6_confidence_threshold, lambda value: CameraState.set_model_confidence_from_str(value, 6)),
            threshold_row("IoU", CameraState.model6_iou_threshold, lambda value: CameraState.set_model_second_threshold_from_str(value, 6)),
            class_name="bg-[#08264f]/80 p-3 rounded-xl w-full space-y-2 border border-cyan-400/25",
        )
        model7_card = rx.box(
            rx.text("Model 7", class_name="text-sm font-semibold text-cyan-50"),
        ...
        return rx.vstack(
            rx.hstack(
                ...
                rx.badge(
                    rx.center(rx.text(f"Model {CameraState.cross_threshold_model}"), width="100%"),
                    variant="surface",
                    min_width="100px",
                    text_align="center",
                ),
